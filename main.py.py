...
# Configuracion para toogle led
led = machine.Pin(2, machine.Pin.OUT) # GPIO2 es el led integrado en muchos ESP32
...
# Montaje SD seguro (si falla, memoria interna)
sd_montada = False
try:
    ...
    try:
        ...
        if oe.args[0] == 17:
            print("Directorio /sd ya existe, continuando.")
        else:
            raise oe

# Configuración WiFi
SSID = "INFINITUMB41F_2.4"
PASSWORD = "Zumi980208"
...
def publicar_mqtt(temp, hum):
    ...
    try:
        ...
        mqtt_client.publish(MQTT_TOPIC, mensaje_completo)
        print(f"Datos enviados MQTT: {mensaje_completo}")
    except Exception as e:
        print("Error al enviar datos MQTT:", e)

# Funcion callback para mensajes entrantes con MQTT
...
while True:
    try:
        mqtt_client.check_msg() # Revisa si hay mensajes entrantes
